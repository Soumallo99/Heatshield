...
|---|---|---|---|
| **Esri ArcGIS World Imagery** (`services.arcgisonline.com`, keyless) | 3D globe imagery + 2D "Satellite" | Esri Terms of Use; public World Imagery service, no key required at this endpoint — review terms before running at scale | "Powered by Esri — Source: Esri, Maxar, Earthstar Geographics, and the GIS User Community" (Cesium credit line + globe footer) |
| **Esri Canvas / Street / Reference services** (`server.arcgisonline.com`, keyless) | 2D "Dark", "Streets", label overlays | as above | credit string on every `<TileLayer>` in `basemaps.js` |
...
| **OpenTopoMap** (keyless) | 2D "Terrain" basemap | CC-BY-SA 3.0 | "© OpenStreetMap contributors · SRTM · © OpenTopoMap (CC-BY-SA)" |
